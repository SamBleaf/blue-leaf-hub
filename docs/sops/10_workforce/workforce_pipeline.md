...
- **Stage length is earned-value-driven**: each stage's duration = its labour value ÷ the team day-rate, **scaled to the number of workers** set on that category (job Schedule tab → the 👷 Workers cell; fewer workers → longer). At full team it equals the budget page's "Days @ margin"; a smaller crew runs proportionally longer. Changing the Workers count (or a stage's start date) recomputes its end. On the job's **Schedule tab** a stage row can be **expanded** to see its budget **subsections** (e.g. First Fix Framing → wall framing, roof framing, window install), each with its own indicative duration derived the same way from its line-item value. The lock control there is a labelled **🔒 Locked / 🔓 Lock** pill. (A stage that was hand-dragged + **locked** keeps its manual span — re-auto-layout only recomputes unlocked stages.)
