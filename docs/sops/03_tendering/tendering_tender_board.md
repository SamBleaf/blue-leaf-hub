...
- **Signature** — your saved email signature (name, title, mobile, website, logo) is added automatically below the sign-off, so you don't paste it into every message. Edit it under **Settings → Email signature → Edit signature**; it is saved **for the whole team** (server-side), so every email uses the same signature no matter who sends it or from which device. *(The signature no longer lives only in one browser — needs migration 157 applied, then one Save to promote it.)*
