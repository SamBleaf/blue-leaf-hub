...
- **Signature** — your saved email signature (name, title, mobile, website, logo) is added automatically below the sign-off. It is **per account** — the email signs as whoever sends it (when Josh sends, it's Josh's signature; when Sam sends, it's Sam's). Edit yours under **Settings → Email signature → Edit signature**; it's saved to your account (server-side) so it works from any device. Staff who haven't set one up inherit the **team default** (Sam's). Admins can tick "Also set as the team default" when saving. *(Needs migrations 157 + 158 applied.)*
- **No reference line in the email** — the RFQ tracking tag (previously a visible "Ref: BLH-RFQ-…" line) is now hidden, so recipients just see your message. Replies still match back to the right RFQ automatically via the email thread.
...
Most subcontractor quote emails match to their RFQ automatically. When one can't be matched (a reply from an unexpected address, a forwarded quote), it lands in **Tendering → Quote Inbox** (a red count on the nav shows how many are waiting). Each row shows the sender, subject, date and the attached PDF at a glance, plus a **suggested match** ("Looks like 2 Forrest Avenue › Structural Steel › JDM Steel — 94%") with the reason. Ways to clear a row:
- **Match** — the suggested job & trade are pre-filled (a strong suggestion opens the match panel ready to go). Confirm the job/RFQ, optionally type the **quote amount (ex GST)**, and **Match quote** — it files the PDF to the job's **INTERNAL / QUOTES** folder and records the quote so it shows on the tender detail and feeds Cost Intelligence, exactly like an auto-matched quote.
- **Dismiss** — for anything that isn't a quote (a statement, spam, a mis-send); add an optional reason. It moves to the **Dismissed** tab instead of sitting in the queue.
- **It's an invoice → Finance** — a mis-sent invoice is handed to the Finance inbox (appears there as unmatched) and cleared from here.
- **＋ New subcontractor** — appears when the sender isn't on file; quick-add them (pre-filled from the email).
- **Tabs & search** — Pending / Matched / Dismissed, with a search box; tick rows to **bulk-dismiss**.
(The same list also still appears under the Quote Tracker's "Unmatched" tab.)
