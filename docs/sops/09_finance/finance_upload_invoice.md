...
### Mobile photo upload (paper invoice on site)
...
- [ ] Finance → Inbox with drop zone visible and a sample invoice listed
- [ ] "extracting" status briefly shown on a newly uploaded document
- [ ] Mobile photo upload button (camera icon) on Finance → Inbox
...
- Only PDF files are accepted via drag-drop — JPG/PNG require the photo upload button
- Maximum upload size is approximately 20 MB; invoices larger than this must be compressed before uploading
- IMAP polling may be disabled in production if IMAP credentials are missing from env vars — invoices will not auto-import
- If the same invoice PDF is uploaded twice, the system does not block it — staff must identify and reject duplicates manually
- The IMAP poller checks only PDF attachments; inline images or non-PDF attachments in emails are ignored
...
**TC-06 — Large file rejection**
1. Attempt to upload a PDF larger than 20 MB
2. Expected: upload is rejected with a file size error message before the upload completes
3. Expected: no row created in `financial_documents`
- [ ] Pass  [ ] Fail  [ ] N/A (no file >20MB available for testing)
