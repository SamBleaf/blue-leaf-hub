...
## 3. What this does
Changes the recipient status from `received` to `accepted`. Mirrors the acceptance to the linked `rfqs` record in the legacy quote tracker. The accepted quote amount flows into the job's cost picture.

## 4. Before you start
- The quote must be recorded (status = `received`)
...
2. Open the trade scope
3. Find the recipient (subcontractor) whose quote you want to accept
4. Click their row to open the **Update quote** panel
5. Change **Status** to `Accepted`
6. Confirm the **Quote amount** is correct (update if needed)
7. Click **Save**
...
| Problem | Solution |
|---------|----------|
| Coverage score not updating after acceptance | Refresh the page — coverage is recalculated on the server and the UI updates on reload |
...
### Pre-test setup
- [ ] An RFQ trade scope with at least 1 recipient in `received` status with a quote_amount set
- [ ] The recipient must have an `rfq_id` set (linked to the rfqs table) — this is set automatically when the RFQ email is sent

### Test cases

**TC-01 — Accept a quote (happy path)**
1. Open a trade scope with a received quote
2. Open the Update quote panel for the subcontractor
3. Change status to `Accepted`
4. Click Save
5. Expected: recipient badge changes to `Accepted` in the UI
6. Expected DB: `rfq_recipients.status = 'accepted'`
7. Expected DB: `rfqs.status = 'accepted'` (if rfq_id linked)
- [ ] Pass  [ ] Fail

**TC-02 — Coverage score updates**
1. Note the package coverage score before accepting any quotes
2. Accept a quote for one trade
3. Expected: coverage score increases (or updates) after acceptance
- [ ] Pass  [ ] Fail
...
4. Expected: status returns to `received`
5. Expected DB: `rfq_recipients.status = 'received'`
- [ ] Pass  [ ] Fail

**TC-05 — Accepted shows in comparison table**
1. Have 2 received quotes, accept one of them
2. Expected: comparison table shows the accepted recipient with `Accepted` badge
3. Expected: the other recipient still shows `received`
- [ ] Pass  [ ] Fail

### Post-test checklist
- [ ] Acceptance recorded in rfq_recipients
- [ ] rfqs row mirrors the acceptance
- [ ] Coverage score recalculates
