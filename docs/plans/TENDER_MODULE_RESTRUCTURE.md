...
- **Decision (now grounded — see [`TENDER_MODEL_A_VS_B.md`](TENDER_MODEL_A_VS_B.md)):** take **A's spine as the base** (proven, populated: 27 RFQs / 8 quotes vs B's ~1 real recipient / 0 quotes) and **absorb B's useful concepts** — trade scopes + addenda — plus the new quote *submission/version* layer (§6.2, new to both). A deliberate synthesis ("Model A extended"), **not** "resurrect Model B" (its tables are empty, unwired, and B doesn't solve multi-quote either).
