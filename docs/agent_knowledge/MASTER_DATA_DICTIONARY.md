...
| `charge_out_hourly` (on charge_up_jobs) | Labour | Versioned (editable per site) | Charge Up detail (site $ rate/hr) | mig 149; a flat per-site charge-out rate that OVERRIDES each worker's `charge_up_hourly` for that site. NULL = per-person rate. Never alters approved hours or cost — charge-out $ stays derived | charge-up per-site + by-FY charge-out / margin (invoicing) | N | N | N |
