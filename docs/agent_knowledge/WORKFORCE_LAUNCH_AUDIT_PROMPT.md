...
**E. Team Directory + Invite** — create/edit/deactivate employee; **email/phone/staff_code** save; **rate shows cents** (80.35 stays 80.35, not 80); BX ID optional. **Invite:** `POST /api/auth/invite` with `{ email, fullName, role, employeeId }` — emails a setup link and, on accept, creates the `user_profiles` row + the canonical employee↔login link (mig 100). Test to `sam+wftest@blueleafbuilding.com.au` only. (The old `POST /api/workforce/employees/:id/invite` was decommissioned in the login-cleanup workstream.) Confirm the worker can then either accept the account invite OR use the magic-link (no account) — both routes to logging hours.
