...
> **Template strategy (decided): one workflow + one master template.** Procurement is the *same process* for every build type — only the item *list* shifts, and only slightly. Rather than N parallel templates (duplication + drift), use **one master list with a per-item `applies_to_build_types` tag** so generation auto-filters (a reno won't get slab/trusses; an extension gets termite). The Buildxact estimate then refines the real job list. The only procurement-significant build-type differences: structural items (slab/frame/trusses → new/KDR/extension), strip-out/demolition (reno/KDR — mostly a subbie service), termite (new/extension), and **"match existing" items (reno/extension) — the highest discontinued/long-lead risk and the main reason the tag exists.**
