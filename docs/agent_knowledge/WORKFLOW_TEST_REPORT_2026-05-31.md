...
### Bugs / issues (prioritised) — STREAM A FIXES APPLIED 2026-05-31
1. **✅ FIXED — CRM module unreachable.** `SalesManager.jsx` read `useParams().tab` but `App.jsx` routes are literal (no `:tab`). Now derives the tab from `useLocation().pathname`. Verified live: `/sales/dashboard`→CrmDashboard, `/sales/contacts`→CrmContacts. (commit aeebdf7)
2. **⚠ REMEDY (dev env, user action) — WHS incidents: `site_reports` missing from the DEV DB.** Confirmed via service-role query: table genuinely absent in dev (swms_templates etc. from the same migration 010 DO exist → partial-apply drift). **Action:** run migration 010's `site_reports` CREATE on the dev Supabase (it's `CREATE TABLE IF NOT EXISTS`, safe), then `NOTIFY pgrst, 'reload schema'`. **Verify prod has `site_reports`** (it should, from a normal 010 apply). No code change.
3. **✅ FIXED — no error boundary + raw-error leaks.** Added a top-level React `ErrorBoundary` (recover screen instead of white-screen). (commit d97d9bd) — Note: the raw-error-string leaks (blueprint billing msg, bad-project-id Postgres strings) are LOW and still open; recommend friendly messages in those catch blocks.
4. **✅ FIXED — role-gated routes bounce to /home on hard load / deep-link.** `AuthContext` had a render gap where `loading` went false before `role` resolved → `RoleRoute` saw role=null and redirected. Added a `profileResolved` flag. Verified: hard nav to `/sales/dashboard` now renders the CRM dashboard. (commit 388940a)
5. **✅ FIXED — KPI mismatch.** Sales header counted `won` leads in active pipeline; home scorecard excludes them. Aligned the Sales header to exclude won. Verified: header now "8 active leads · $5.6m · 1 won", matching home. (commit 760f888)
