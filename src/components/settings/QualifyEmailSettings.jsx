...
// Admin-editable copy for the two Qualify-stage client emails (intro + 7-day follow-up), stored
// server-side (user_settings/crm_qualify_email). Both fall back to the approved defaults if blank.
// Merge tokens are literal — no client free-text is inserted. Grounded in the website's "build
// conversation" language; keep the CONTENT.txt voice (warm, plain, no "dream home / seamless").
const SAMPLE = {
  "{{client_first_name}}": "Jane",
  "{{project_type}}": "renovation",
  "{{project_suburb}}": "Malvern",
  "{{cal_booking_link}}": "https://cal.com/blue-leaf-build/build-conversation",
  "{{sender_phone}}": "0434 046 399",
  "{{user_signature}}": "Kind regards,\nSam Morris\nDirector\n0434 046 399",
};
const applySample = (s) => Object.entries(SAMPLE).reduce((acc, [k, v]) => acc.split(k).join(v), s || "");

export default function QualifyEmailSettings() {
  ...
  useEffect(() => {
    let stop = false;
    (async () => {
      const { ok, data } = await apiFetch("/api/sales/qualify-email-template");
      if (stop) return;
      if (ok) {
        setTpl({
  ...
  async function save() {
    ...
    const { ok, error: e } = await apiPost("/api/sales/qualify-email-template", payload);
    setSaving(false);
    if (ok) setNote("Saved — the Qualify emails will use this copy.");
  ...
  return (
    <section id="qualify-email" className="scroll-mt-24 rounded-card border border-hairline bg-surface p-6 shadow-sm">
      <h2 className="text-lg font-semibold text-primary">Qualify emails</h2>
      <p className="mt-1 text-sm text-muted max-w-2xl">
        The two client emails at the Qualify stage: the <strong>introduction</strong> (sent when you trigger it — attaches
        the company profile) and the <strong>follow-up</strong> (auto-sent 7 days later if they haven&apos;t booked or replied).
        Merge tokens are literal: {placeholders.map((p) => <code key={p} className="mx-0.5 rounded bg-page px-1 text-xs">{p}</code>)}
      </p>
