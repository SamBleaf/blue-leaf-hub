...
function DiaryTab({ job }) {
  // Tasks (job TasksPanel) + the shared site-diary component (same carpentry_site_diary table,
  // now also reused by a BLB Charge Up site). Behaviour is unchanged from the inline version.
  return (
    <div className="p-6">
      <TasksPanel jobId={job.id} />
      <hr className="border-hairline -mx-6 mb-6" />
      <CarpentrySiteDiary diaryBase={`/api/carpentry/jobs/${job.id}`} address={job.address} />
    </div>
  );
}
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-b-card border border-t-0 border-hairline">
        ...
        {tab === "safety"   && <WhsPackTab jobId={job.id} />}
