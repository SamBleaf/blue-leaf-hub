...
Defined in `.Codex/commands/` — invoke with `/check`, `/ship`, `/sprint`:
...
- `registerModule4Routes` — RFQ engine: Codex extraction, PO generation, Buildexact sync
...
- API: `POST /api/sales/leads/:id/conversations/analyse` — sends transcript to Codex (model: `Codex-opus-4-5`) with APB extraction prompt, returns `{ suggestions }` as structured JSON (stage, qualifying score updates, next action, notes)
...
**Schedule generation:** AI generates tasks from project description via Codex in `module6Routes.mjs`. Ripple cascade (`previewRipple()` in `scheduleUtils.js`) propagates date shifts to downstream tasks via `depends_on` array. `RippleWarningModal` shown before applying.
